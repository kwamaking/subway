...
var irchandler = exports.irchandler = function(socket) {
  ...
  socket.on('connect', function(data) {
    ...
    if(client === undefined) {
      client = new irc.Client('irc.snoonet.com', data.nick, {
        port: 6697,
    ...
    socket.on('command', function(text) { console.log(text); client.send(text.join(" ")); });

    socket.on('whois', function(text) { console.log(text); client.send("WHOIS" + text);});
